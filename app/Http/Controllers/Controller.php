...
class Controller extends BaseController
{
    ...
    public function render($view, $additional = [], $perms = [])
    {
        $user = auth()->user();
        $permission = $this->data['permission'] ?? '';
        // dd($this->perms, $permission, $perms);
        $has_perms = 0;
        foreach ($perms as $perm) {
            if ($user->checkPerms($this->perms . '.' . $perm)) {
                $has_perms += 1;
            }
        }
        if (count($perms) && !$has_perms) {
            return abort(403);
        }
        $this->prepare($additional);
        if (empty($permission) || $user->checkPerms($permission)) {
    ...
    {
        if ($params === 'name:num') {
            $params = 'name:num|label:#|width:20px';
        } elseif ($params === 'name:action') {
            $params = 'name:action|label:Aksi|width:40px|sortable:false';
        } elseif ($params === 'name:created_by') {
            $params = 'name:created_by|label:Dibuat Oleh|width:150px|sortable:false';
        } elseif ($params === 'name:created_at') {
            $params = 'name:created_at|label:Dibuat Pada|width:150px|sortable:false';
        } elseif ($params === 'name:updated_by') {
            $params = 'name:updated_by|label:Diperbarui|width:130px|sortable:false';
        } elseif ($params === 'name:updated_at') {
            $params = 'name:updated_at|label:Diperbarui Pada|width:150px|sortable:false';
                ...
                if (!empty($item['type'])) {
                    ...
                    switch ($item['type']) {
                        ...
                        case 'detail.show':
                            $default['icon'] = 'fa fa-eye text-primary';
                            $default['label'] = 'Lihat';
                            $default['url'] = route($this->prepared('routes') . '.detail.show', $item['id']);
                            $default['attrs'] = 'data-modal-backdrop="true"';
                            break;
                        case 'detail':
                            $default['icon'] = 'fa fa-plus text-primary';
                            $default['label'] = 'Detail';
                            $default['url'] = route($this->prepared('routes') . '.detail', $item['id']);
                            $default['attrs'] = 'data-modal-backdrop="true"';
                            break;
                        case 'approval':
                        ...
                        case 'detail.edit':
                            $default['icon'] = 'fa fa-edit text-success';
                            $default['label'] = 'Ubah';
                            $default['url'] = route($this->prepared('routes') . '.detail.edit', $item['id']);
                            break;
                        case 'print':
                        ...
                        case 'detail.delete':
                            ...
                            $default['label'] = 'Hapus';
                            $default['class'] = 'base-modal--confirm';
                            $default['url'] = route($this->prepared('routes') . '.detail.destroy', $item['id']);
                            if (!empty($item['text'])) {
                                $confirm = __('Hapus data') . ' ' . $item['text'] . '?';
                                $default['attrs'] = 'data-confirm-text="' . $confirm . '"';
                            }
