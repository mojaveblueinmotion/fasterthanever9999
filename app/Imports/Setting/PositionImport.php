    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $rw => $row) {
            ...
            if (!empty($name)) {
                ...
                $struct = Struct::subbranch()->where('name', $subbranch)->first();
                if (!$struct) {
                    // Check Cabang
                    $struct = Struct::branch()->where('name', $branch)->first();
                    if (!$struct) {
                        // Check Department
                        $struct = Struct::department()->where('name', $department)->first();
                        if (!$struct) {
                            // Check Divisi
                            $struct = Struct::division()->where('name', $division)->first();
                            if (!$struct) {
                                // Check Bod
                                $struct = Struct::bod()->where('name', $bod)->first();
                                if (!$struct) {
                                    // Check Boc
                                    $struct = Struct::boc()->where('name', $boc)->first();
                                }
                            }
                        }
                    }
                ...
                // Check stuct
                if (!$struct) {
                    throw new \Exception('MESSAGE--Unit Kerja untuk Jabatan "' . $name . '" tidak tersedia!', 1);
                }
