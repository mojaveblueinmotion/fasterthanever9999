...
class PolisMotor extends Model
{
    ...
    protected $table = 'trans_polis_motor';
    ...
    {
        return $this->hasOne(PolisMotorCek::class, 'polis_id');
    }

    public function detailNilai()
    {
        return $this->hasOne(PolisMotorNilai::class, 'polis_id');
    }

    public function detailClient()
    {
        return $this->hasOne(PolisMotorClient::class, 'polis_id');
    }

    public function detailPayment()
    {
        return $this->hasOne(PolisMotorPayment::class, 'polis_id');
    }
    ...
    public function handleSubmitSave($request)
    {
        $this->beginTransaction();
        try {
            $menu = \App\Models\Setting\Globals\Menu::where('module', 'asuransi.polis-motor')->first();
    ...
    {
        $data = 'Data Polis Motor dengan No Asuransi ' . $this->no_asuransi;
