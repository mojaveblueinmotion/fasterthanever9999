...
class Base
{
    ...
    public static function getModulesWithOutSubMenu($moduleName = false)
    {
        $modules = [];
        foreach (config('backendmenu') as $menu) {
            if (!empty($menu['name']) && !empty($menu['title'])) {
            ...
            if (empty($menu['section'])) {
                ...
                if (!empty($menu['submenu'])) {
                    foreach ($menu['submenu'] as $sub1) {
                        if (!empty($sub1['perms'])) {
                            $modules[] = [
                                'perms' => $menu['perms'] . '.' . $sub1['perms'],
                                'title' => __($menu['title']) . '/' . __($sub1['title'])
                            ];
                        }
                        // Submenu 2
                        if (!empty($sub1['submenu'])) {
                            foreach ($sub1['submenu'] as $sub2) {
                                if (!empty($sub2['perms'])) {
                                    $modules[] = [
                                        'perms' => $sub2['perms'],
    ...
    public static function renderAsideMenu($menu)
    {
        ...
        if (!empty($menu['section'])) {
            ...
            foreach ($menu['submenu'] as $item) {
                // dd($menu['perms'].'.'.$item['perms'] . '.view');
                if (isset($item['perms']) && !$user->hasPermissionTo($menu['perms'].'.'.$item['perms'] . '.view')) {
                    continue;
                }
