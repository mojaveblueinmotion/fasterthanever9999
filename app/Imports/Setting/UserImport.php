    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $rw => $row)
        {
            ...
            // Check Location
            $struct = Struct::whereName($locationName)->first();
            if (!$struct) {
                throw new \Exception('MESSAGE--Unit Kerja: "'.$locationName.'" tidak tersedia!', 1);
            }
            // Check Position
            $position = $struct->positions()->whereName($positionName)->first();
            if (!$position) {
                throw new \Exception('MESSAGE--Jabatan: "'.$positionName.'" dengan Unit Kerja: "'.$locationName.'" tidak tersedia!', 1);
            }
